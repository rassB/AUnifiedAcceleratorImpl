...
void kernel_mapping_c(KerType initial_kernel[mr][c][k][k],
	KerType flat_kernel[mr][c * k * k])
    {
...
void input_mapping_naive_c(ImgType initial_input[c][h][l],
	ImgType flat_input[c * k * k][h * l])
...
//TODO write this sanity check in a more elegant way; that would avoid writing if it's to overwrite just after.
		    if (!(x + icol < l))
			{
			std::cout << "non-existing input element [" << i << ","
				<< j << "," << x + icol << "] value="
				<< initial_input[i][j][x + icol]
				<< " is being replaced by[" << i << "," << j + 1
				<< "," << (x + icol) - l << "] value="
				<< initial_input[i][j + 1][(x + icol) - l]
				<< " is being put in flat["
				<< kernel_channel_count << "," << icol << ']'
				<< std::endl;
			flat_input[kernel_channel_count][icol] =
				initial_input[i][j + 1][x + icol - l];
			if (!(j + 1 < h))
			    {
			    std::cout
				    << " INNER SANITY :non-existing input element ["
				    << i << "," << j + 1 << "," << x + icol - l
				    << "] value="
				    << initial_input[i][j + 1][x + icol - l]
				    << " is being replaced by[" << i + 1 << ","
				    << j + 1 - h << "," << (x + icol) - l
				    << "] value="
				    << initial_input[i + 1][j + 1 - h][(x + icol)
					    - l] << " is being put in flat["
				    << kernel_channel_count << "," << icol
				    << ']' << std::endl;
			    flat_input[kernel_channel_count][icol] =
				    initial_input[i + 1][j + 1 - h][x + icol - l];
			    if (!(i + 1 < c))
				{
				std::cout
					<< "   INNER SANITY :non-existing input element ["
...
void altinput_mapping_naive_c(ImgType initial_input[c][h][l],
	ImgType flat_input[c * h * k][l])
    {
	...
	for (int j = 0; j < h; ++j)
		...
		for (int x = 0; x < l; ++x)
		    {
		    (!(x + shifter < l)) ?
			    flat_input[count][x] = ImgType_ZERO :
			    flat_input[count][x] = initial_input[i][j][x
				    + shifter];
...
void partial_matrix_mapping(ImgType initial_input[c][h][l],
	ImgType flat_input[c * k * k][mc + 2 * pad])
    {
...
void kernel_mapping_c2(KerType initial_kernel[m][c][k][k],
	xf::cv::Window<m, c * k, KerType> kmap[k])
    {
    //64 ×3 kernels with a size of 3 ×3 are mapped to a rearranged matrix with dimensions of 64 ×(3 ×3 ×3).  (they use 64 mc in their impl.)
    //void kernel_mapping_c(KerType initial_kernel[mr][c][k][k],KerType flat_kernel[mr][c*k*k]);
    int mr_counter = 0;
	...
	for (int j = 0; j < c; ++j)
		...
		for (int y = 0; y < k; ++y)
		    {
		    kmap[y].insert(initial_kernel[i][j][x][y], i, mr_counter); //&initial_kernel[i][j][x][y];
...
void mapwindow_c2(ImgType B[c][h][l],
	...
	for (int Wrow = 0; Wrow < k; ++Wrow)
		...
		{
		featureBuffer[cpy].insert_row(&B[chan][Wrow][0],
			Wrow + chan * c); //B will be streamed, not implemented yet.
		}
	    }
	}
